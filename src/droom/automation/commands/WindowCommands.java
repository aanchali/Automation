...
public class WindowCommands {
	...
	public static void switchToNewWindow(WebDriver myDriver) {
		try {
			String parentHandle = myDriver.getWindowHandle();

			for (String popUpHandle : myDriver.getWindowHandles()) {
	 ...
	 * @param locator
	 */
	public static void switchToFrame(WebDriver myDriver, HSSFSheet sheet1,
			...
			if (yes.equals(execute)) {
				...
				if (myElement.isDisplayed() == true) {
					if (myElement.isEnabled() == true) {
						...
						// new
						// Actions(myDriver).moveToElement(myElement).perform();
						myDriver.switchTo().frame(myElement);
	...
	public static void switchToFrameAndWaitForNextElement(WebDriver myDriver,
	...
	public static void returnToParentWindow(WebDriver myDriver) {
		try {
